import { Play, Pause, Download, Lock, X, Music2 } from 'lucide-react';
import { useMemo } from 'react';
// ⚠️ CORREGIDO: Usamos ../../ (dos niveles atrás) para llegar a src/config
...
export function LatestUploads({ tracks, selectedGenre, onGenreSelect, user, onPlay, currentTrack, isPlaying }: LatestUploadsProps) {
    ...
    <section id="latest" className="py-12 bg-black min-h-[400px]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 border-b border-white/10 pb-6 gap-4">
          ...
          <p className="text-[10px] font-bold text-gray-600 uppercase tracking-widest">Database: {filteredTracks.length} tracks</p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-2">
          {filteredTracks.length > 0 ? (
            ...
            <div className="py-20 text-center border border-dashed border-white/10 rounded-2xl"><p className="text-gray-600 font-black uppercase tracking-[0.2em] text-xs">{tracks.length === 0 ? 'Loading Database...' : 'No tracks found for this filter'}</p></div>
          )}
        </div>
      </div>
    </section>
